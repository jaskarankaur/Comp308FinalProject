...
module.exports = function(app) {
   ...
   // app.route('/displayComments').get(users.displayComments);
    // Set up the 'users' base routes
	app.route('/users').post(users.create).get(users.list);


    
    app.route('/users/nurse')
        .post(users.createNurse);

	// Set up the 'users' parameterized routes 
   // app.route('/users/comment').get(users.read).post(users.commentByUsername);
	  // .put(users.update)
   ...
   // app.route('/edit').post(users.updateByUsername);
    ...
    app.route('/read_nurse').post(users.nurseByUsername);
